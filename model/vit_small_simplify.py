...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.norm_embed = norm_layer(embed_dim) if norm_layer else nn.Identity()
...
class VisionTransformer_posembed(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
        ...
        # pos embed fix
        self.pos_proj = nn.Linear(27, embed_dim, bias=False)
        self.learned_pos_embed = nn.Parameter(torch.randn(1, 196, embed_dim) * 0.02)
    ...
    # pos embed fix V0
    # def _pos_embed_fix(self, x: torch.Tensor, pos:torch.Tensor) -> torch.Tensor:
    #     b,l,f = x.shape
    #     print(f'x shape: {x.shape}')
    #     print(f'pos shape: {pos.shape}')
    #     pos = pos.reshape(b,l,9,3)
    #     print(f'pos shape 1: {pos.shape}')
    #     pos = pos[:,:,4,:]
    #     print(f'pos shape 2: {pos.shape}')
    #     print(f'pos 2: {pos}')
    #     pos = pos.sum(2)

    #     pos = (pos-torch.mean(pos))/(torch.std(pos))
    #     if self.pos_embed is None:
    #         return x.view(x.shape[0], -1, x.shape[-1])
    #     pos_embed = pos
    #     to_cat = []
    ...
    #     if self.reg_token is not None:
    #         to_cat.append(self.reg_token.expand(x.shape[0], -1, -1))
    #     x = x + pos_embed.unsqueeze(2)
    #     if to_cat:
    #         x = torch.cat(to_cat + [x], dim=1)

    #     return self.pos_drop(x)

    # pos embed fix V1 with linear projection
    # def _pos_embed_fix(self, x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
    #     b, l, f = x.shape
        
    #     # 1. Process position vectors
    #     pos = pos.to(x.dtype)  # (B,196,27)
        
    #     # 2. Project 27D -> 768D (embed_dim)
    #     pos_embed = self.pos_proj(pos)  # (B,196,768)
        
    #     # # 3. Normalize position embeddings
    #     # pos_embed = (pos_embed - torch.mean(pos_embed, dim=-1, keepdim=True)) / \
    #     #             (torch.std(pos_embed, dim=-1, keepdim=True) + 1e-6)
        
    #     # 4. Add to patch embeddings
    #     x = x + pos_embed
        
    #     # 5. Prepend tokens (BEFORE position addition)
    #     to_cat = []
    #     if self.cls_token is not None:
    #         cls_tokens = self.cls_token.expand(b, -1, -1)
    #         to_cat.append(cls_tokens)
    #     if self.reg_token is not None:
    #         reg_tokens = self.reg_token.expand(b, -1, -1)
    #         to_cat.append(reg_tokens)
        
    #     if to_cat:
    #         x = torch.cat(to_cat + [x], dim=1)  # (B, 1+reg_tokens+196, 768)
    
    #     return self.pos_drop(x)

    # # pos embed fix V2 with repitition
    # def _pos_embed_fix(self, x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
    #     b, l, f = x.shape  # (B, 196, 768)
    #     pos_dim = pos.size(-1)  # 27
        
    #     # Calculate how many times to repeat 27D to reach 768
    #     num_repeats = f // pos_dim  # 768//27 = 28
    #     remainder = f % pos_dim  # 768%27 = 12
        
    #     # Repeat the positional vectors
    #     pos_embed = pos.repeat(1, 1, num_repeats)  # (B, 196, 27*28=756)
        
    #     # Handle remainder by taking first 12 elements
    #     if remainder > 0:
    #         pos_embed = torch.cat([pos_embed, pos[..., :remainder]], dim=-1)  # (B,196,768)
    ...
    #     # Prepend class/reg tokens (without positional info)
    #     to_cat = []
    #     if self.cls_token is not None:
   ...
   # pos embed fix V3 with repitition
    def _pos_embed_fix(self, x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
        b, l, f = x.shape  # (B, 196, 768)
        pos_dim = pos.size(-1)  # 27
        
        pos_embed = self.pos_proj(pos) + self.learned_pos_embed  # Combine fixed + learned
        ...
        # Add positional info to tokens:
        if to_cat:
            token_pos = torch.zeros(b, len(to_cat), self.embed_dim).to(x.device)
            pos_embed = torch.cat([token_pos, pos_embed], dim=1)  # (B, 1+reg+196, 768)
            x = torch.cat(to_cat + [x], dim=1) + pos_embed  # Add after token concat
...
class VisionTransformer_distillation(nn.Module):
    dynamic_img_size: Final[bool]
    def __init__(
            self,
            img_size: Union[int, Tuple[int, int]] = 224,
        ...
        self.head_drop = nn.Dropout(drop_rate)
        self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
        self.subhead1 = nn.Linear(self.embed_dim, 2048)
        self.subhead2 = nn.Linear(2048, num_classes)
        self.init_weights(weight_init)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # x = self.fc_norm(x)
        x = self.head_drop(x)
        x_out1 = self.subhead1(x)
        x_out2 = self.subhead2(x_out1)
